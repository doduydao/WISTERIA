...
# Model V2
# ============================================================
class TemporalRelationModelV2(nn.Module):
    def __init__(self,
                 bert,
                 tokenizer,
                 hidden_ffn_dim,
        ...
        # Late fusion weight
        self.logit_mixer = nn.Parameter(torch.tensor([0.4, 0.4, 0.2]))
    ...
    def _label_aware_fusion(self, ctx_pair):
        ...
        label = self.label_emb  # (C,H)
        label_exp = label.unsqueeze(0).expand(B, -1, -1)      # (B,C,H)
        ctx_exp = ctx_pair.unsqueeze(1).expand(-1, C, -1)     # (B,C,H)
    
        z = torch.cat([ctx_exp, label_exp], dim=-1)   # (B,C,2H)
        z_flat = z.view(B*C, -1)                      # (B*C, 2H)
        gates = torch.sigmoid(self.gate_fc(z_flat))   # (B*C,1)
        gates = gates.view(B, C)                      # (B,C)
        return gates

    def _late_fusion(self, logits_list):
        w = torch.softmax(self.logit_mixer, dim=0)  # (3,)
        return w[0] * logits_list[0] + w[1] * logits_list[1] + w[2] * logits_list[2]
    ...
    def forward(self,
        ...
        """
        spacy_masks: [B, M, L] (bool)
        entity_mask_*: [B, L] (bool)
        """
        ...
        # Lấy e1, e2, word_embs (cần có hàm apply_entity_and_spacy_masks do bạn viết riêng)
        e1_embs, e2_embs, word_embs = apply_entity_and_spacy_masks(
            embeddings, entity_mask_e1, entity_mask_e2, spacy_masks
        )
        ...
        # Cross-attn
        # (b) Mask special tokens (E1/E2/CLS/SEP) theo tokenizer
        special_token_ids = set(self.tokenizer.all_special_ids)
        special_token_strs = set(self.tokenizer.convert_ids_to_tokens(list(special_token_ids)))
        # tạo mask theo spaCy token text
        special_mask = torch.zeros_like(word_valid_mask, dtype=torch.bool)
        if spacy_tokens is not None:
            for i, toks in enumerate(spacy_tokens):
                m_lim = min(len(toks), special_mask.size(1))
                for j in range(m_lim):
                    if toks[j].text in special_token_strs:
                        special_mask[i, j] = True
                    # bonus: nếu dùng tokenizer dạng "[E1]" "[E2]" có thể kiểm tra substring
                    elif toks[j].text.upper() in ["[E1]", "[E2]", "[/E1]", "[/E2]"]:
                        special_mask[i, j] = True
    
        # (c) Gộp lại: True = bị che khỏi attention
        combined_mask = pad_words | special_mask  # (B,M)
        combined_mask = combined_mask.to(self.device)

            
        q_e1, attn_e1 = self.cross_e1(e1_embs.unsqueeze(1), word_ctx, kv_mask=combined_mask)
        q_e2, attn_e2 = self.cross_e2(e2_embs.unsqueeze(1), word_ctx, kv_mask=combined_mask)
        q_pair, attn_pair = self.cross_pair(h_pair.unsqueeze(1), word_ctx, kv_mask=combined_mask)

        attn_e1 = attn_e1.squeeze(1)
        attn_e2 = attn_e2.squeeze(1)
        attn_pair = attn_pair.squeeze(1)

        # Context vectors
        ctx_e1, idx_e1, val_e1 = self._topk_context(attn_e1, word_ctx, ~combined_mask, k_override=top_k)
        ctx_e2, idx_e2, val_e2 = self._topk_context(attn_e2, word_ctx, ~combined_mask, k_override=top_k)
        ctx_pair, idx_pair, val_pair = self._topk_context(attn_pair, word_ctx, ~combined_mask, k_override=top_k)
        ...
        logits_fusion = self.cls_fusion(fusion_inp)
        logits_context = self.context_head(torch.cat([ctx_e1, ctx_e2, ctx_pair], dim=-1))

        # Apply gates
        logits_fusion = logits_fusion * gates
        logits_context = logits_context * gates

        logits = self._late_fusion([logits_fusion, logits_biaff, logits_context])
        ...
        if return_aux_logits:
            return logits, [logits_fusion, logits_biaff, logits_context]
